...
__global__ void
__launch_bounds__(ms_cuda::kMaxThreadsPerBlock, 1)
...
void NormalizeImplCUDA(mshadow::Stream<gpu> *s,
    ...
    <<<N, dim3(ms_cuda::kMaxThreadsPerBlock, 1), 0, stream>>>(input, output,
...
void NormalizeBackwardImplCUDA(mshadow::Stream<gpu> *s,
    ...
    // 1 image per block. N is batch size.
    <<<N, dim3(ms_cuda::kMaxThreadsPerBlock, 1), 0, stream>>>(out_grad, in_grad,
