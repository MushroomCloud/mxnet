...
void CastStorageDnsRspGPUImpl_(const OpContext& ctx,
  ...
  const dim_t threads_per_block = mshadow::ms_cuda::kBaseThreadNum;
...
struct CastDnsCsrIndPtrWarpKernel {
  ...
  __device__ __forceinline__ static void Map(int tid,
    ...
    const dim_t warps_per_block = mshadow::ms_cuda::kBaseThreadNum / 32;
...
struct CastDnsCsrIndPtrBlockKernel {
  ...
  __device__ __forceinline__ static void Map(int tid,
                                             ...
                                             const nnvm::dim_t num_rows,
                                             const nnvm::dim_t num_cols) {
    using mshadow::ms_cuda::kBaseThreadNum;
    using nnvm::dim_t;
