...
void DepthwiseConv2dForwardGpu(mshadow::Stream<gpu> *stream,
  ...
  if (CanLaunchDepthwiseConv2dGPUSmall(args)) {
    ...
    int block_num = std::min(num_output/mshadow::ms_cuda::kBaseThreadNum + 1,
                             mshadow::ms_cuda::kMaxGridNum);
    ...
    if (args.filter_height == 3 && args.filter_width == 3) {
      DepthwiseConv2dForwardKernel<DType, 3, 3>
          <<<block_num, mshadow::ms_cuda::kBaseThreadNum, 0, s>>>(data.dptr_,
                                                               weight.dptr_,
                                                               args,
                                                               num_output,
                                                               out.dptr_);
...
void DepthwiseConv2dBackwardDataGpu(mshadow::Stream<gpu> *stream,
  ...
  if (CanLaunchDepthwiseConv2dGPUSmall(args)) {
    ...
    auto s = mshadow::Stream<gpu>::GetStream(stream);
    int block_num = std::min(num_in_grad/mshadow::ms_cuda::kBaseThreadNum + 1,
                             mshadow::ms_cuda::kMaxGridNum);
    DepthwiseConv2dBackwardDataKernel<DType>
        <<<block_num, mshadow::ms_cuda::kBaseThreadNum, 0, s>>>(args,
                                                             out_g.dptr_,
                                                             weight.dptr_,
                                                             in_data_g.dptr_,
                                                             num_in_grad);
